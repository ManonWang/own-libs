    ...
    <div class="analyse_div"> 
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    <th></th>
            <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?><th>
                    <?php echo ($deptData[$deptId]['name']); ?>
                <?php $colStat[$deptId] = array_sum($riskData); ?>
                </th><?php endforeach; endif; ?>
            <th>全段</th>
            </tr>
            </thead>
            <tbody>
            <?php if(is_array($levelData)): foreach($levelData as $levelKey=>$levelItem): ?><tr>
                    <td><?php echo ($levelItem['name']); ?></td>
                <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?><td>
                        <?php echo ($riskData[$levelItem['id']]); ?>
                    <?php $rowStat[$levelKey] = $rowStat[$levelKey] + $riskData[$levelItem['id']]; ?>
                    </td><?php endforeach; endif; ?>
                <td><?php echo ($rowStat[$levelKey]); ?></td>
                </tr><?php endforeach; endif; ?>
            <tr>
                <td>合计</td>
            <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?><td><?php echo ($colStat[$deptId]); ?></td><?php endforeach; endif; ?>
            <td><?php echo array_sum($colStat);?></td>
            </tr>
            <tr>
                <td>警示</td>
            <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?><td>
                <?php $alarm = get_star($riskData, $levelData); ?>
                <?php $__FOR_START_1938206456__=0;$__FOR_END_1938206456__=$alarm['red'];for($i=$__FOR_START_1938206456__;$i < $__FOR_END_1938206456__;$i+=1){ ?><span class="glyphicon glyphicon-star" style="color:red"></span><?php } ?>
                <?php $__FOR_START_626969458__=0;$__FOR_END_626969458__=$alarm['orange'];for($i=$__FOR_START_626969458__;$i < $__FOR_END_626969458__;$i+=1){ ?><span class="glyphicon glyphicon-star" style="color:orange"></span><?php } ?>
                <?php $__FOR_START_52228944__=0;$__FOR_END_52228944__=$alarm['yellow'];for($i=$__FOR_START_52228944__;$i < $__FOR_END_52228944__;$i+=1){ ?><span class="glyphicon glyphicon-star" style="color:yellow"></span><?php } ?>
                </td><?php endforeach; endif; ?>
            <td></td>
            </tr>
            </tbody>
