...
<html lang="zh-CN">
  ...
  <body>
    ...
    <p class="page-title"><?php echo date("Y年m月d日", $startTime);?>至<?php echo date("Y年m月d日", $endTime);?>安全风险问题提报平台分析</p>
    <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
        <?php if(is_array($data)): foreach($data as $menuCat=>$menuCatItem): if(is_array($menuCatItem)): foreach($menuCatItem as $menuType=>$menuTypeItem): ?><tr>
                    <td><?php echo ($risk_menu[$menuCat]['child'][$menuType]); ?></td>
                <?php if(is_array($levelData)): foreach($levelData as $levelKey=>$levelItem): ?><td>
                        <?php echo ($menuTypeItem[$levelItem['id']]); ?>
                        <?php $colStat[$levelKey] = $colStat[$levelKey] + $menuTypeItem[$levelItem['id']]; ?>
                    </td><?php endforeach; endif; ?>
                <td><?php echo array_sum($menuTypeItem);?></td>
