...
<table class="table table-bordered form-table">
    <tr>
        ...
        <td>
            <?php echo ($alarmType['supervise']['name']); ?>
            <input type="hidden" name="type" value="<?php echo ($alarmType['supervise']['id']); ?>" />
        </td>
    </tr>
    <tr>
        <td class="feild-title">预警时间</td>
        <td ><input type="text" class="form-control" name="alarm_time" id="alarm_time" value="<?php echo ($data['alarm_time']); ?>"/></td>
    </tr>
    <tr>
        <td class="feild-title">落实时限</td>
        <td ><input type="text" class="form-control" name="fix_time" id="fix_time" value="<?php echo ($data['fix_time']); ?>"/></td>
    </tr>
    <tr>
        <td class="feild-title">预警来源</td>
        <td >
            <select name="alarm_source" class="form-control">
              <?php if(is_array($dicLists[$allTypes['alarm_source']])): foreach($dicLists[$allTypes['alarm_source']] as $key=>$item): ?><option value="<?php echo ($item['id']); ?>" <?php if($item['id'] == $data['alarm_source']): ?>selected="selected"<?php endif; ?>><?php echo ($item['name']); ?></option><?php endforeach; endif; ?>
            </select>        
        </td>
    </tr>
    <tr>
        ...
        <td >
            <select name="fix_dept[]" class="form-control" id="fix_dept" multiple="multiple">
              <?php if(is_array($dicLists[$allTypes['department']])): foreach($dicLists[$allTypes['department']] as $key=>$item): ?><option value="<?php echo ($item['id']); ?>"<?php if(in_array($item['id'], $data['fix_dept'])): ?>selected="selected"<?php endif; ?>><?php echo ($item['name']); ?></option><?php endforeach; endif; ?>
            </select>
        </td>
    </tr>
    <tr>
        <td class="feild-title">预警主题</td>
        <td ><input type="text" class="form-control" name="alarm_title" value="<?php echo ($data['alarm_title']); ?>" /></td>
    </tr>
    <tr>
        <td class="feild-title">预警内容</td>
        <td >
            <textarea class="form-control" rows="5" cols="45" name="alarm_content"><?php echo ($data['alarm_content']); ?></textarea>
        </td>
    </tr>
    <tr>
        <td class="feild-title">整改要求</td>
        <td >
            <textarea class="form-control" rows="5" cols="45" name="fix_method"><?php echo ($data['fix_method']); ?></textarea>
        </td>
    </tr>
    <tr>
        ...
        <td >
            <label for="alarm_store_2">
                <input type="radio" name="alarm_store" id="alarm_store_2" value="2" <?php if(2 == $data['alarm_store'] or 0 == $data['alarm_store']): ?>checked="checked"<?php endif; ?>>库内
            </label> &nbsp;
            <label for="alarm_store_1">
                <input type="radio" name="alarm_store" id="alarm_store_1" value="1" <?php if(1 == $data['alarm_store']): ?>checked="checked"<?php endif; ?>>库外
            </label> 
        </td>
    </tr>
</table>

<?php if(0 == $data['id']): ?><div class="text-center">
