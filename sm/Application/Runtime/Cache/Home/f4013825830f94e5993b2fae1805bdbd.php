...
<html lang="zh-CN">
  ...
  <head>
    ...
    <title>数据填报统计 -- 机车质量跟踪 -- 研判预警</title>
  </head>
...
<div id="breadcrumb">
 <ol class="breadcrumb">
  ...
  <li class="active">数据填报统计</li>
 </ol>    
</div>

<p class="page-title">机车质量跟踪(本月)</p>
<table class="table table-striped table-bordered table-hover">
  <thead>
    <tr>
        <th>机车车型</th>
        <?php if(is_array($types)): foreach($types as $type=>$item): ?><th><?php echo ($type); ?></th><?php endforeach; endif; ?>
    </tr>
  </thead>
  <tbody>
    <tr>
        <td>jt-6</td>
        <?php if(is_array($types)): foreach($types as $type=>$item): ?><td><?php echo ($live6[$type]); ?></td><?php endforeach; endif; ?>
    </tr>
    <tr>
        <td>jt-28</td>
        <?php if(is_array($types)): foreach($types as $type=>$item): ?><td><?php echo ($live28[$type]); ?></td><?php endforeach; endif; ?>
    </tr>
  </tbody>
</table>
