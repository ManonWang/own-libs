                ...
                <ul class="dropdown-menu multi-level second-menu" role="menu">
                    <?php if(is_array($risk_menu)): foreach($risk_menu as $cat_key=>$cat_item): ?><li class="dropdown-submenu">
                        ...
                        <ul class="dropdown-menu thrid-menu">
                          <?php if(is_array($cat_item['child'])): foreach($cat_item['child'] as $type_key=>$type_item): ?><li><a tabindex="-1" href="/risk/add.html?menu_cat=<?php echo ($cat_key); ?>&menu_type=<?php echo ($type_key); ?>"><?php echo ($type_item); ?></a></li>
                             <?php if($type_key != count($cat_item['child'])): ?><li class="divider"></li><?php endif; endforeach; endif; ?>
                        </ul>
                    </li>
...
<table class="table table-striped table-bordered table-hover">
  <thead>
    <tr>
        <th></th>
        <?php if(is_array($data)): foreach($data as $dept_id=>$risk_data): ?><th><?php echo ($deptData[$dept_id]['name']); ?></th>
        <?php $stat[$dept_id] = array_sum($risk_data); endforeach; endif; ?>
        <th>全段</th>
    </tr>
  </thead>
  <tbody>
        <?php if(is_array($levelData)): foreach($levelData as $level_key=>$level_item): ?><tr>
            <?php $row = 0;?>
            <td><?php echo ($level_item['name']); ?></td>
            <?php if(is_array($data)): foreach($data as $dept_id=>$risk_data): ?><td><?php echo ($risk_data[$level_item['id']]); ?></td>
            <?php $row += $risk_data[$level_item['id']]; endforeach; endif; ?>
            <td><?php echo ($row); ?></td>
        </tr><?php endforeach; endif; ?>
        <tr>
            <td>合计</td>
            <?php if(is_array($data)): foreach($data as $dept_id=>$risk_data): ?><td><?php echo ($stat[$dept_id]); ?></td><?php endforeach; endif; ?>
