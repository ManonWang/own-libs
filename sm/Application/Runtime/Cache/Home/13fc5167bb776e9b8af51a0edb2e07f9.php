...
<html lang="zh-CN">
  ...
  <body>
        ...
        <form id="query-form" class="form-inline">
            <div class="form-group">
                ...
                <select name="resp_dept_id" class="form-control" id="resp_dept_id">
                    ...
                    <?php if(is_array($dicLists[$allTypes['department']])): foreach($dicLists[$allTypes['department']] as $key=>$item): ?><option value="<?php echo ($item['id']); ?>" <?php if($item['id'] == $respDeptId): ?>selected="selected"<?php endif; ?>><?php echo ($item['name']); ?></option><?php endforeach; endif; ?>
                </select>
            </div> 
            &nbsp;&nbsp;
            <div class="form-group">
    ...
    <p class="page-title"><?php echo date("Y年m月d日", $startTime);?>至<?php echo date("Y年m月d日", $endTime);?>安全风险问题构成分析</p>
    ...
    $(function(){
        ...
        var option = {
            ...
            legend: {
                orient: 'vertical',
                top: 'bottom',
                data: [<?php if(is_array($data)): foreach($data as $key=>$item): ?>'<?php echo ($dicLists[$allTypes['risk_item']][$key]['name']); ?>',<?php endforeach; endif; ?>]
            },
            series : [
                {
                    name: '问题占比',
                    ...
                    center: ['50%', '60%'],
                    data:[
                        <?php if(is_array($data)): foreach($data as $key=>$item): ?>{value:<?php echo ($item); ?>, name:'<?php echo ($dicLists[$allTypes['risk_item']][$key]['name']); ?>'},<?php endforeach; endif; ?>
                    ],
                    itemStyle: {
                        emphasis: {
                            shadowBlur: 10,
