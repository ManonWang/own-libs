...
<table class="table table-striped table-bordered table-hover">
  ...
  <tbody>
        <?php if(is_array($levelData)): foreach($levelData as $levelKey=>$levelItem): ?><tr>
            <td>
                <?php echo ($levelItem['name']); ?>
                <?php if(is_array($data)): foreach($data as $deptId=>$riskData): $rowStat[$levelKey] = $rowStat[$levelKey] + $riskData[$levelItem['id']]; endforeach; endif; ?>
            </td>
            <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?><td>
                <?php $precent = round($riskData[$levelItem['id']] * 100 / $rowStat[$levelKey], 1); $precent = $precent > 0 ? $precent . '%' : ''; ?>
                <?php echo ($precent); ?>
            </td><?php endforeach; endif; ?>
        </tr><?php endforeach; endif; ?>
        <tr>
            <?php $total = array_sum($colStat); ?>
            <td>总占比</td>
            <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?><td><?php echo round($colStat[$deptId] * 100 / $total, 1);?>%</td><?php endforeach; endif; ?>
        </tr>
  </tbody>
</table>
...
<script>
    $(function(){
        ...
        var option = {
            ...
            legend: {
                data:[<?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?>'<?php echo ($deptData[$deptId]['name']); ?>',<?php endforeach; endif; ?>]
            },
            grid: {
                left: '3%',
                right: '4%',
            ...
            xAxis: {
                type: 'category',
                boundaryGap: false,
                data:[<?php if(is_array($levelData)): foreach($levelData as $levelkey=>$levelItem): ?>'<?php echo ($levelItem['name']); ?>',<?php endforeach; endif; ?>]
            },
            yAxis: {
                type: 'value',
                axisLabel: {
            ...
            series: [
                <?php if(is_array($data)): foreach($data as $deptId=>$riskData): ?>{
                    name:'<?php echo ($deptData[$deptId]['name']); ?>',
                    type:'line',
                    stack: '占比',
                    data:[
                            <?php if(is_array($levelData)): foreach($levelData as $levelkey=>$levelItem): echo round($riskData[$levelItem['id']] * 100 / $rowStat[$levelkey], 1);?>,<?php endforeach; endif; ?>
                    ]
