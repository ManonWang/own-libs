...
<html lang="zh-CN">
  ...
  <head>
    ...
    <title>天气情况分布分析 -- 数据分析预警 -- 研判预警</title>
  </head>
  ...
  <body>
        ...
        <form id="query-form" class="form-inline">
            <div class="form-group">
                ...
                <select name="weather_type" class="form-control" id="weather_type">
                    ...
                    <?php if(is_array($dicLists[$allTypes['weather']])): foreach($dicLists[$allTypes['weather']] as $key=>$item): ?><option value="<?php echo ($item['id']); ?>" <?php if($item['id'] == $weatherType): ?>selected="selected"<?php endif; ?>><?php echo ($item['name']); ?></option><?php endforeach; endif; ?>
                </select>
            </div> 
            &nbsp;&nbsp;
            <div class="form-group">
    ...
    <p class="page-title"><?php echo date("Y年m月d日", $startTime);?>至<?php echo date("Y年m月d日", $endTime);?>安全风险问题天气情况分布分析</p>
