...
<form id="query-form" class="form-inline">
  ...
  <div class="form-group">
        ...
        <select name="resp_dept_id[]" class="form-control" id="resp_dept_id" multiple="multiple">
            <?php if(is_array($dicLists[$allTypes['department']])): foreach($dicLists[$allTypes['department']] as $key=>$item): ?><option value="<?php echo ($item['id']); ?>" <?php if(in_array($item['id'], $resp_dept_id)): ?>selected="selected"<?php endif; ?>><?php echo ($item['name']); ?></option><?php endforeach; endif; ?>
        </select>
  </div> 
  &nbsp;&nbsp;
  <div class="form-group">
...
<table class="table table-striped table-bordered table-hover">
  <thead>
    <tr>
        <th></th>
        <?php if(is_array($data["risk_dept"])): foreach($data["risk_dept"] as $deptId=>$riskData): ?><th>
  ...
  <tbody>
      <tr>
          <td>自查均量</td>
          <?php if(is_array($data['risk_dept'])): foreach($data['risk_dept'] as $deptId=>$riskData): ?><td>
            <?php echo round($data['self_found'][$deptId] / $data['staff_count'][$deptId], 2);?>
          </td><?php endforeach; endif; ?>
      </tr>
      <tr>
          <td>外检均量</td>
          <?php if(is_array($data['risk_dept'])): foreach($data['risk_dept'] as $deptId=>$riskData): ?><td>
            <?php echo round($data['other_found'][$deptId] / $data['staff_count'][$deptId], 2);?>
          </td><?php endforeach; endif; ?>
      </tr>
  </tbody>
</table>
...
<script>
    $(function(){
        ...
        var option = {
            ...
            yAxis: {
                type: 'category',
                data: [<?php if(is_array($data["risk_dept"])): foreach($data["risk_dept"] as $deptId=>$riskData): ?>'<?php echo ($deptData[$deptId]['name']); ?>',<?php endforeach; endif; ?>]
            },
            series: [
                {
                    name: '自查',
                    ...
                    data: [<?php if(is_array($data['risk_dept'])): foreach($data['risk_dept'] as $deptId=>$riskData): echo round($data['self_found'][$deptId] / $data['staff_count'][$deptId], 2);?>,<?php endforeach; endif; ?>]
                },
                {
                    name: '外检',
                    type: 'bar',
                    ...
                    data: [<?php if(is_array($data['risk_dept'])): foreach($data['risk_dept'] as $deptId=>$riskData): echo round($data['other_found'][$deptId] / $data['staff_count'][$deptId], 2);?>,<?php endforeach; endif; ?>]
                },
            ]
        };
