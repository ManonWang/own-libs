...
<html lang="zh-CN">
  ...
  <head>
    ...
    <title>JT28分析 -- 机车质量跟踪 -- 研判预警</title>
  </head>
...
<div class="inline-form">
  <form id="query-form" class="form-inline">
    <div class="form-group">
       <label for="query_time">检修时间</label>
       <select class="form-control" name="timeYear"> 
        <?php $__FOR_START_1676870730__=date('Y');$__FOR_END_1676870730__=date('Y') - 30;for($index=$__FOR_START_1676870730__;$index > $__FOR_END_1676870730__;$index+=-1){ ?><option value="<?php echo ($index); ?>" <?php if($index == $timeYear): ?>selected="selected"<?php endif; ?> ><?php echo ($index); ?></option><?php } ?>
       </select>
       年
       <select class="form-control" name="timeMonth"> 
        <option value="">选择月</option>
        <?php $__FOR_START_157413733__=1;$__FOR_END_157413733__=13;for($index=$__FOR_START_157413733__;$index < $__FOR_END_157413733__;$index+=1){ ?><option value="<?php echo ($index); ?>" <?php if($index == $timeMonth): ?>selected="selected"<?php endif; ?> ><?php echo sprintf('%02d', $index);?></option><?php } ?>
       </select>
       月
    </div>
    <div class="form-group">
...
<p class="page-title">机车质量跟踪系统-JT28分析</p>
<table class="table table-striped table-bordered table-hover">
  <caption class="text-center"><strong>电力机车</strong></caption>
  <thead>
    <tr>
        <th>车型</th>
        <?php if(is_array($dicLists[$allTypes['live_own']])): foreach($dicLists[$allTypes['live_own']] as $key=>$item): ?><th><?php echo ($item['name']); ?></th><?php endforeach; endif; ?>
    </tr>
  </thead>
  <tbody>
    <?php if(is_array($data[$typeData['power']['id']])): foreach($data[$typeData['power']['id']] as $type=>$power): ?><tr>
        <td><?php echo ($type); ?></td>
        <?php if(is_array($dicLists[$allTypes['live_own']])): foreach($dicLists[$allTypes['live_own']] as $key=>$item): ?><td><?php echo ($power[$item['id']]); ?></td><?php endforeach; endif; ?>
    </tr><?php endforeach; endif; ?>
  </tbody>
</table>

<table class="table table-striped table-bordered table-hover">
  <caption class="text-center"><strong>内燃机车</strong></caption>
  <thead>
    <tr>
        <th>车型</th>
        <?php if(is_array($dicLists[$allTypes['live_own']])): foreach($dicLists[$allTypes['live_own']] as $key=>$item): ?><th><?php echo ($item['name']); ?></th><?php endforeach; endif; ?>
  ...
  <tbody>
    <?php if(is_array($data[$typeData['diesel']['id']])): foreach($data[$typeData['diesel']['id']] as $type=>$diesel): ?><tr>
        <td><?php echo ($type); ?></td>
        <?php if(is_array($dicLists[$allTypes['live_own']])): foreach($dicLists[$allTypes['live_own']] as $key=>$item): ?><td><?php echo ($diesel[$item['id']]); ?></td><?php endforeach; endif; ?>
    </tr><?php endforeach; endif; ?>
  </tbody>
</table>
