    ...
    <div class="analyse_div"> 
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    <th></th>
            <?php if(is_array($data['risk_data'])): foreach($data['risk_data'] as $deptId=>$riskData): ?><th>
                    <?php echo ($deptData[$deptId]['name']); ?>
                <?php if(is_array($riskData)): foreach($riskData as $riskLevelId=>$riskLevelCount): $score = $scoreMap[$dicLists[$allTypes['risk_level']][$riskLevelId]['user_key']] * $riskLevelCount; ?>
                    <?php $colStat[$deptId] = $colStat[$deptId] + $score; endforeach; endif; ?>
            ...
            <?php if(is_array($levelData)): foreach($levelData as $levelKey=>$levelItem): ?><tr>
                ...
                <?php if(is_array($data['risk_data'])): foreach($data['risk_data'] as $deptId=>$riskData): ?><td>
                        <?php echo round($riskData[$levelItem['id']] * $scoreMap[$levelKey] / $data['staff_count'][$deptId], 2);?>
                    </td><?php endforeach; endif; ?>
                </tr><?php endforeach; endif; ?>
            <tr>
                <td>合计</td>
            <?php if(is_array($data['risk_data'])): foreach($data['risk_data'] as $deptId=>$riskData): ?><td><?php echo round($colStat[$deptId] / $data['staff_count'][$deptId], 2);?></td><?php endforeach; endif; ?>
            </tr>
            </tbody>
        </table>
    </div>
