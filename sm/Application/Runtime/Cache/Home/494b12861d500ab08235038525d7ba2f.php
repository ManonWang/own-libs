...
<div id="breadcrumb">
 <ol class="breadcrumb">
  ...
  <li class="active">重点甄别</li>
 </ol>    
</div>

<p class="page-title">集宁机务段每日重点安全信息甄选统计表</p>
<form id="add-form">
<table class="table table-striped table-bordered table-hover">
  ...
  <tbody>
    <?php if(is_array($pagedList['data_list'])): foreach($pagedList['data_list'] as $key=>$item): ?><tr>
        <td style="width:80px;"><?php echo ($item['id']); ?></td>
        ...
        <td style="width:150px;"><?php echo ($dicLists[$allTypes['department']][$item['resp_dept_id']]['name']); ?></td>
        <td><?php echo ($dicLists[$allTypes['risk_item']][$item['risk_outline_id']]['name']); ?></td>
        <td style="width:120px">
            <label for="stress_trace_<?php echo ($item['id']); ?>">
                <input type="radio" name="stress_trace_<?php echo ($item['id']); ?>" id="stress_trace_<?php echo ($item['id']); ?>" 
                       value="1"  <?php if($item['stress_trace'] == 1): ?>checked="checked"<?php endif; ?> >是
            </label>
            <label for="stress_trace_<?php echo ($item['id']); ?>">
                <input type="radio" name="stress_trace_<?php echo ($item['id']); ?>" id="stress_trace_<?php echo ($item['id']); ?>" 
                       value="2"  <?php if($item['stress_trace'] == 2): ?>checked="checked"<?php endif; ?> >否
            </label>
        </td>
...
<div class="text-center">
    <input type="botton" value="提交" class="btn btn-primary action-btn"  action="/handle/selectDeal" reload="true"/>
</div>
</form>
