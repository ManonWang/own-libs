...
<form id="query-form" class="form-inline">
  <div class="form-group">
      ...
      <select name="type" class="form-control" id="type">
        ...
        <option value="2" <?php if(2 == $type): ?>selected="selected"<?php endif; ?>>提报部门</option>
      </select>
  </div> 
  &nbsp;&nbsp;
  <div class="form-group">
...
<?php
 unset($levelData['level_1']); $level_1 = $levelData['level_1']['id']; $level_2 = $levelData['level_2']['id']; $level_3 = $levelData['level_3']['id']; $level_4 = $levelData['level_4']['id']; $level_5 = $levelData['level_5']['id']; $level_6 = $levelData['level_6']['id']; ?>
<p class="page-title"><?php echo date("Y年m月d日", $startTime);?>至<?php echo date("Y年m月d日", $endTime);?>安全风险问题提报<?php echo ($source); ?>分析</p>
<table class="table table-striped table-bordered table-hover">
  <thead>
    <tr>
        <th>提报<?php echo ($source); ?></th>
        <?php if(is_array($levelData)): foreach($levelData as $key=>$level_item): ?><th><?php echo ($level_item['name']); ?></th><?php endforeach; endif; ?>
        <th>合计</th>
    </tr>
  </thead>
  <tbody>
    <?php if($type != 1): if(is_array($data)): foreach($data as $dept_id=>$item): ?><tr>
            <td><?php echo ($deptData[$dept_id]['name']); ?></td>
            <?php if(is_array($levelData)): foreach($levelData as $level_id=>$level_item): ?><td><?php echo ($item[$level_item['id']]); ?></td>
            <?php $stat[$level_id] = $item[$level_item['id']]; endforeach; endif; ?>
            <td><?php echo array_sum($item);?></td>
        </tr><?php endforeach; endif; ?>
    <?php else: ?>
        <?php if(is_array($data)): foreach($data as $cat_id=>$item_1): if(is_array($item_1)): foreach($item_1 as $type_id=>$item_2): ?><tr>
                    <td><?php echo ($risk_menu[$cat_id]['child'][$type_id]); ?></td>
                    <td><?php echo ($item_2[$level_1]); ?></td>
                    <td><?php echo ($item_2[$level_2]); ?></td>
                    <td><?php echo ($item_2[$level_3]); ?></td>
                    <td><?php echo ($item_2[$level_4]); ?></td>
                    <td><?php echo ($item_2[$level_5]); ?></td>
                    <td><?php echo ($item_2[$level_6]); ?></td>
                    <td><?php echo array_sum($item_2);?></td>
                </tr>
                <?php  $stat['level_1'] += $item_2[$level_1]; $stat['level_2'] += $item_2[$level_2]; $stat['level_3'] += $item_2[$level_3]; $stat['level_4'] += $item_2[$level_4]; $stat['level_5'] += $item_2[$level_5]; $stat['level_6'] += $item_2[$level_6]; endforeach; endif; endforeach; endif; endif; ?>
    <tr>
        <td>合计</td>
        <?php if(is_array($levelData)): foreach($levelData as $level_id=>$level_item): ?><td><?php echo ($stat[$level_id]); ?></td><?php endforeach; endif; ?>
        <td><?php echo array_sum($stat);?></td>
    </tr>
  </tbody>
</table>
