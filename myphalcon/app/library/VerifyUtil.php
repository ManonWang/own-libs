...
class VerifyUtil {
    ...
    public function check($code, $id = '') {
        ...
        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && $this->storage($key, null);
            return true;
        }
    ...
    /**
     * 输出验证码并把验证码的值保存
     * 验证码保存的格式为： array('verify_code' => '验证码值', 'verify_time' => '验证码创建时间');
     * @access public
    ...
    public function entry($id = '') {
        ...
        if ($this->useZh) { // 中文验证码
            for ($i = 0; $i < $this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, mt_rand(-20, 20), $this->fontSize * ($i + 1) * 1.5, $this->fontSize + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        } else {
            for ($i = 0; $i < $this->length; $i++) {
        ...
        $secode = array();
        $secode['verify_code'] = $code; // 把校验码保存
        $secode['verify_time'] = time();  // 验证码创建时间
        $this->storage($key . $id, $secode);
     ...
     * 获取字体
     */
    private function _setCodeFont() {
        $ttfPath = PUBLIC_PATH . $this->ttfPath . ($this->useZh ? 'zh-ttf' : 'en-ttf') . '/';
     ...
     * 存储
     */
    private function storage($key, $val = '') {
        session_start();
        return '' === $val ? $_SESSION[$key] : $_SESSION[$key] = $val;
    }
