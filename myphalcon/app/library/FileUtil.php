...
class FileUtil {
     ...
     * 获取文件信息
     * @param $filename
     * @param $need_stat 是否需要统计信息
     * @return array
     */
    public static function getFileInfo($filename, $need_stat = false) {
        $fileInfo = pathinfo($filename);
        if ($need_stat) {
            $statInfo = array_slice(stat($filename), 13);
            $fileInfo = array_merge($fileInfo, $statInfo);
        }
        return $fileInfo;
    }
     ...
     * @return number
     */
    public static function write($filename, $data, $append = true) {
        $dirname = dirname($filename);
        self::mkdir($dirname);
        return $append ? file_put_contents($filename, $data, FILE_APPEND) : file_put_contents($filename, $data);
    }
